...
- **Where things live:** `ime/` service and controller; `layout/` key data and the layers
  ...
  prefs and the settings screen (and the Languages and Custom words screens and Backup section); `backup/` the
...
- **Languages** are data in `layout/Languages.kt`: rows, accents, native name. The phone board
  ...
  custom words and backups over. The globe key exists only with two or more languages enabled; a
  first tap goes back to `Settings.previousLanguage` (`GlobeTap.LAST_USED`, the default, as on an
  iPhone; `setCurrentLanguage` stores the language it replaces) and taps in a row after it go on
  through `Languages.next`, all the way round, until another key or a new field ends the run; with
  `NEXT` (as on Gboard) every tap goes on. A choice on the Languages screen from three languages up. The persisted
...
- **Generated, never hand-edited:** `app/build/`, `.gradle/`.
  `app/src/main/res/raw/dependencies.txt` is committed but written by
  `./gradlew :app:generateDependencyLicences` (see Licence below).

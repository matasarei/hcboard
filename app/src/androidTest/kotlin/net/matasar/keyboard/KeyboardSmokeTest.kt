...
class KeyboardSmokeTest {
    ...
    fun typesHello() {
        ...
        repeat(6) {
            ...
            device.waitForIdle()
            Thread.sleep(1_500)
            shown = device.executeShellCommand("dumpsys input_method").contains("mIsInputViewShown=true")
        }
        assertTrue("keyboard never showed", shown)

        for (letter in "hello") tapLetter(letter)
        device.waitForIdle()
        Thread.sleep(500)

        val typed = device.findObject(By.clazz("android.widget.EditText")).text

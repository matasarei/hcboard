...
class KeyMetricsTest {
    ...
    fun `both zones shrink with the key and neither ever disappears`() {
        ...
        assertTrue(Dimens.legendTextSize(eightyPercent).value >= 6.5f)
        assertTrue(Dimens.glyphSize(landscape).value >= 11f)
    }

    @Test
    fun `a word label never outgrows the glyph beside it`() {
        for (height in listOf(24.dp, 26.dp, 36.8.dp, 42.dp, 46.dp, 56.dp)) {
